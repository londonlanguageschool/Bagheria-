...
async function saveStudentForm(event) {
  ...
  try{
    ...
    // V2.5 FIX: the Students "Class" column is only a legacy display field.
    // Real class membership lives in Enrolments, so editing an existing
    // student must create/end enrolments as well as update the student.
    if(classId){
      const portalData = await llsApiGet("getPortalData");
      const activeEnrolments = (portalData.enrolments || []).filter(item =>
        String(item["Student ID"] || "").trim() === studentId &&
        String(item["Status"] || "").trim().toLowerCase() === "active"
      );

      const selectedClass = (state.classes || []).find(item =>
        String(item.id || "").trim() === String(classId).trim()
      );
      const selectedSchoolYear = String(
        selectedClass?.schoolYear || schoolYear || "2026-27"
      ).trim();

      const sameClass = activeEnrolments.find(item =>
        String(item["Class ID"] || "").trim() === String(classId).trim() &&
        String(item["School Year"] || "").trim() === selectedSchoolYear
      );

      if(!sameClass){
        // End any other active class enrolment before moving the student.
        for(const existing of activeEnrolments){
        ...
        enrolment = {
          success:true,
          enrolmentId:String(sameClass["Enrolment ID"] || "")
        };
      }
    }
    ...
    if(isConversion){
      ...
      if(!enrolment) throw new Error("Class enrolment was not created.");
  ...
  try {
    ...
    state.students = rows.map((r) => ({
      ...
      classId: (() => {
        ...
        return match ? String(match.id || "") : rawClass;
      })(),
...
window.addEventListener("load", async () => {
  await llsLoadClassesFromSheets();
  await llsLoadStudentsFromSheets();
  await llsLoadEnquiriesFromSheets();
});
